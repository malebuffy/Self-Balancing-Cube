...
void setup() {
    SerialBT.begin("ESP32_BT_Motors"); // Initialize Bluetooth serial
    ...
    SerialBT.println("Bluetooth setup complete");
    delay(2000);
}
...
void loop() {
    ...
    if (currentT - previousT_1 >= 100) {
        ...
        case 1:
            digitalWrite(BRAKE, HIGH);
            if (lock) SerialBT.println("Rotating motor 1.");
            lock = 0;
            Motor1_control(-50);
            break;
        case 2:
            digitalWrite(BRAKE, LOW);
            if (lock) SerialBT.println("Stop.");
            lock = 0;
            Motor1_control(0);
            break;
        case 3:
            digitalWrite(BRAKE, HIGH);
            if (lock) SerialBT.println("Change direction...");
            lock = 0;
            Motor1_control(50);
            break;
        case 4:
        ...
        case 5:
            digitalWrite(BRAKE, HIGH);
            if (lock) SerialBT.println("Checking encoder...");
            lock = 0;
            Motor1_control(-70);
            break;
        case 6:
            ...
            if (lock && motor1_speed > 300) {
                SerialBT.println("Encoder OK.");
                SerialBT.print("Speed: ");
                SerialBT.println(motor1_speed);
                SerialBT.println("Stop.");
            } else if (lock && motor1_speed <= 0) {
                SerialBT.println("Encoder FAIL.");
                SerialBT.print("Speed: ");
                SerialBT.println(motor1_speed);
                SerialBT.println("Stop.");
            }
        ...
        case 7:
            digitalWrite(BRAKE, HIGH);
            if (lock) SerialBT.println("Rotating motor 2.");
            lock = 0;
            Motor2_control(-50);
            break;
        case 8:
        ...
        case 13:
            digitalWrite(BRAKE, HIGH);
            if (lock) SerialBT.println("Rotating motor 3.");
            lock = 0;
            Motor3_control(-50);
            break;
        case 14:
